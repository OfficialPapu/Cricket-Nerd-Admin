...
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    $query = "SELECT * FROM user_registration WHERE `Full Name` LIKE ? OR `Email` LIKE ? ORDER BY ID ASC";
    ...
    if ($result->num_rows == 0) {
        $_SESSION['toast'] = "User Does Not Exits";
    }
} else {
...
<head>
    ...
    <title>User List | The Cricket Nerd</title>
...
<body class="bg-gray-100" style="background-image: url('Media/Logo/bg-cn.jpg'); background-size: cover; background-position: center;">
    ...
    <div class="container mx-auto py-8 px-1">
       <h1 class="text-2xl font-bold text-center text-white mt-10 mb-4">All Users | The Cricket Nerd</h1>
        <form method="GET" action="" class="mb-6 flex flex-col sm:flex-row gap-2">
            <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search by name or email" class="border rounded-lg px-4 py-2 flex-grow">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full table-fit text-sm">
                    <thead>
                        ...
                        <th class="px-8 py-6 text-left">Created Time</th>
                        </tr>
                    </thead>
                        ...
                        <?php if ($result && $result->num_rows > 0): ?>
                            <?php while ($row = $result->fetch_assoc()): ?>
                                <tr class="border-b hover:bg-gray-100">
                                    <td class="px-8 py-6"><?php echo htmlspecialchars($row['ID']); ?></td>
                                    <td class="px-8 py-6"><?php echo htmlspecialchars($row['Full Name']); ?></td>
                                    ...
                                    <td class="px-8 py-6"><?php echo htmlspecialchars($row['Create Time']); ?></td>
                                </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr>
